...
def parse_matrix_code(matrix_code):
    try:
        ...
        lot_code = (
            segments[-1].rstrip("Z").strip()
        )  # Stripping 'Z' and trimming whitespace
...
class BarcodeScannerApp:
    def __init__(self, root):
        ...
        self.undo_button = tk.Button(
            root, text="Undo Last Scan", command=self.undo_last_scan
        )
        ...
        self.export_as_button = tk.Button(
            root, text="Export As...", command=self.export_as
        )
        ...
        # Add drop down box to select between mouser or digikey
        self.provider_dropdown = tk.StringVar()
        self.provider_dropdown.set("mouser")  # default value
        self.provider_dropdown_menu = ttk.Combobox(
            root, textvariable=self.provider_dropdown
        )
        self.provider_dropdown_menu["values"] = ("mouser", "digikey")
        self.provider_dropdown_menu.grid(row=4, column=0, padx=10, pady=10)

    def add_barcode(self, event=None):
        barcode_data = self.barcode_entry.get().strip()
        if barcode_data:
            part_number, mfr_part_number, description, lot_code = parse_matrix_code(
                barcode_data
            )
            if part_number and mfr_part_number and description and lot_code:
                ...
                self.barcode_listbox.insert(
                    tk.END,
                    f"{sanitized_part_number} - {sanitized_mfr_part_number} - {sanitized_description} - {sanitized_lot_code}",
                )
    ...
    def export_to_excel(self):
        ...
        df["Provider"] = self.provider_dropdown.get()
